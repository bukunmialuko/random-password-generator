...
public class RandomPasswordGeneratorOld {
    ...
    public String generatePassayPassword() {
        ...
        lengthRule.setMaximumLength(10);
        lengthRule.setMaximumLength(8);
        return gen.generatePassword(12, splCharRule, lowerCaseRule, upperCaseRule, digitRule);
    }


    public String generatePassayPasswordWithRules(HashMap<String, Object> config) {
        PasswordGenerator gen = new PasswordGenerator();
        CharacterData lowerCaseChars = EnglishCharacterData.LowerCase;
        CharacterRule lowerCaseRule = new CharacterRule(lowerCaseChars);
        lowerCaseRule.setNumberOfCharacters((Integer) config.get("noOfLowerCase"));
        CharacterData upperCaseChars = EnglishCharacterData.UpperCase;
        CharacterRule upperCaseRule = new CharacterRule(upperCaseChars);
        upperCaseRule.setNumberOfCharacters((Integer) config.get("noOfUpperCase"));
        CharacterData digitChars = EnglishCharacterData.Digit;
        CharacterRule digitRule = new CharacterRule(digitChars);
        digitRule.setNumberOfCharacters(2);
        CharacterData specialChars = new CharacterData() {
        ...
        CharacterRule splCharRule = new CharacterRule(specialChars);
        splCharRule.setNumberOfCharacters((Integer) config.get("numberOfSpecialCharacters"));
        LengthRule lengthRule = new  LengthRule();
        lengthRule.setMinimumLength((Integer) config.get("minLength"));
        lengthRule.setMaximumLength((Integer) config.get("maxLength"));
        return gen.generatePassword((Integer) config.get("minLength"), splCharRule, lowerCaseRule, upperCaseRule, digitRule);
    }
...
//    }
    ...
    public String generateSecureRandomPasswordWithRules(HashMap<String, Object> config) {
        Stream<Character> pwdStream = Stream.concat(getRandomNumbers(2),
                Stream.concat(getRandomSpecialChars(2),
                        Stream.concat(getRandomAlphabets(2, true),
                                getRandomAlphabets(4, false))));
